...
parser.add_argument(
    "-i", "--input", help="File containing the dataset's file paths"
)
parser.add_argument(
    "-l", "--limit", 
...
parser.add_argument(
    "-tr", "--threshold-range", nargs="+", action="extend", 
    help="creates an evenly spaced range over a specified interval. \
        Args: start, stop, number of values", type=float
)
...
def verify_emotion_thresholds(analyze_output, actual_emotion, threshold):
    ...
    return (dominant_emotion == actual_emotion) and \
...
def compute_emotions_against_thresholds(thresholds = [0]):
    ...
    for threshold in thresholds: correct_emotion_count[threshold] = 0
    ...
    for line in lines:
        ...
        if (is_TUTFS(folder_name)):
            actual_emotion = \
                TUTFS_emotion_codes[int(os.path.basename(line)[-5])]
        else:
            actual_emotion = \
                KDEF_emotion_codes[os.path.basename(line)[-7 : -5]]

        for threshold in thresholds: 
            correct_emotion_count[threshold] += \
                verify_emotion_thresholds(analyze_output, actual_emotion, threshold)
